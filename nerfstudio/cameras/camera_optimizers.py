...
class CameraOptimizer(nn.Module):#bara in pas mide ke ye forward kochik tar az in class ijad shoe va ino toy  class asli seda mizane
...
#این روش یک تانسور از شاخص های دوربین را به عنوان ورودی می گیرد.
    def forward(
        self,
        ...
        if self.config.mode == "off":
            pass
        elif self.config.mode == "SO3xR3":#hame ro dare b joz yekish
            outputs.append(exp_map_SO3xR3(self._get_pose_adjustment()[indices, :]))
        elif self.config.mode == "SE3":#hamero shamel mish
            outputs.append(exp_map_SE3(self._get_pose_adjustment()[indices, :]))
        else:
            assert_never(self.config.mode)
        # Detach non-trainable indices by setting to identity transform
        if self.non_trainable_camera_indices is not None:#این بخش بررسی می کند که آیا شاخص های دوربین غیر قابل آموزش مشخص شده است یا خیر.
...
@dataclass
class ScaledCameraOptimizerConfig(CameraOptimizerConfig):#کلاس پیکربندی برای بهینه‌ساز مقیاس‌پذیر دوربین
    """Configuration of axis-masked optimization for camera poses."""

    _target: Type = field(default_factory=lambda: ScaledCameraOptimizer)#هدف پیش فرض برای کلاس تعریف میکند
...
class ScaledCameraOptimizer(CameraOptimizer):
    ...
    def get_loss_dict(self, loss_dict: dict) -> None:
        ...
        if self.config.mode != "off":
            ...
            #جریمه انتقال به صورت عنصر به عنصر اعمال می‌شود، در حالی که جریمه چرخش به نرم اجزای چرخش اعمال می‌شود.
            loss_dict["camera_opt_regularizer"] = (
